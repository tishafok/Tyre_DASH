...
#LAPTIME FILTER AND MAX LAP STINT ESTIMATION
LAP_FILTER = 115 
MAX_LAPS = 14 
            ...
            for line in msg:
                ...
                if line.startswith('$P'):
                    ...
                    if len(telem_dict[car]) == 0:
                        telem_dict[car].append([TOD, speed, lap_time, throttle, gear, brake])
                    else:
...
class DashThread(threading.Thread):
    ...
    def run(self):
        ...
        def get_fuel(n_intervals)
            ...
            if lap_times and telem_dict:
                
                for car in list(lap_times.keys()):
                    if len(lap_times[car])<2 or len(telem_dict[car])<2:
                        continue
                            
                    columns_mpg = ['TOD', 'Speed', 'LapTime', 'Throttle', 'Gear', 'Break']
                    df_mpg = pd.DataFrame(telem_dict[car], columns=columns_mpg)
